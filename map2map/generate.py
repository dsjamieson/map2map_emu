...
def worker(local_rank, node, args):
    ...
    generate_dataset = GenerateFieldDataset(
        ...
        pk_pattern=args.gen_pk_pattern,
        out_pattern=args.gen_out_pattern,
    ...
    if args.no_dis :
        ...
        d2d_model = import_attr("nbody.StyledVNet", models, callback_at=args.callback_at)
        d2d_model = d2d_model(style_size, in_chan, out_chan, scale_factor=args.scale_factor, **args.misc_kwargs)
        d2d_model.to(device)
    ...
    if args.no_vel :
        ...
        v2v_model = import_attr("nbody.StyledVNet", models, callback_at=args.callback_at)
        v2v_model = v2v_model(style_size, in_chan, out_chan, scale_factor=args.scale_factor, **args.misc_kwargs)
        v2v_model.to(device)
