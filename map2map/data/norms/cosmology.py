...
def dis(x, undo=False, z=0.0, Om=0.31, dis_std=6.0, **kwargs):
    dis_norm = dis_std * D(z, Om=Om)  # [Mpc/h]
...
def vel(x, undo=False, z=0.0, Om=0.31, dis_std=6.0, **kwargs):
    vel_norm = dis_std * D(z, Om=Om) * H(z, Om=Om) * f(z, Om=Om) / (1 + z)  # [km/s]
